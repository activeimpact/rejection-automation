...
def load_env_file(env_file_path):
    ...
    try:
        ...
        logger.info(f"Successfully loaded {len(env_vars)} variables from .env file")
        return env_vars
...
logger.info(f"Loaded COPPER_EMAIL: {env_vars.get('COPPER_EMAIL', 'NOT FOUND')}")
if 'COPPER_API_TOKEN' in env_vars:
    token = env_vars['COPPER_API_TOKEN']
    logger.info(f"Loaded COPPER_API_TOKEN: {token[:4]}...{token[-4:] if len(token) > 8 else ''}")
else:
    logger.warning("COPPER_API_TOKEN not found in .env file")
...
# Set up API credentials directly from env_vars
COPPER_API_TOKEN = env_vars.get('COPPER_API_TOKEN', '')
COPPER_EMAIL = env_vars.get('COPPER_EMAIL', '')
...
COPPER_API_URL = env_vars.get('COPPER_API_URL', 'https://api.copper.com/developer_api/v1')
...
# Try to get secrets from Streamlit first, fall back to environment variables or our loaded .env
def get_secret(key, default=None):
    """Get a secret from Streamlit secrets, loaded .env, environment variables with fallback to default."""
    try:
    ...
    # Try our manually loaded env_vars first
    if key in env_vars:
        return env_vars[key]
...
def generate_rejection_email(lead_details, reason):
    ...
    try:
        # Format the template with the lead details - removed the initial "Thanks" line
...
def send_email(recipient_email, subject, body, cc_email=None):
    ...
    try:
        ...
        error_msg = f"Sender refused: {str(e)}. This may be due to restrictions on your email account."
        logger.error(error_msg)
        return False, error_msg
